...
addpath('audios', 'Functions', 'videos2', 'street', 'stopandgo', 'long');
...
m1=max(max_f);
m2=min(max_f);
start_A= round((m1-m2)/4);

%% START A mesurat per la mitjana de la f màxima en els primers tres segons
start_max=[];
start_A_vect=[0];
threshold=[0];
...
subplot(3,1,3);
plot(t2, filtered_vect*1000);%max(max_f));
% hold on
% plot(t2, result_cars*600);
hold on
plot(t2, max_f);
% hold on
% plot(t2, start_A_vect);
hold on
plot(t2, threshold);
hold off
xlabel('Time (mm:ss)');
